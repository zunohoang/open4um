...
interface SlideCanvasProps {
    ...
    onUpdateComponent: (id: string, patch: Partial<SlideComponent>, recordHistory?: boolean) => void;
...
export const SlideCanvas = ({
    ...
    const handleStartMove = (e: React.MouseEvent, comp: SlideComponent) => {
        ...
        if (comp.id === selectedCompId && isText && target.closest("[data-text-content='true']")) {
            return;
        }
    ...
    const handleStartResize = (
        ...
        const compEl =
            (e.currentTarget.closest(`[data-component-id='${comp.id}']`) as HTMLElement) ||
        ...
        const handleMouseMove = (e: MouseEvent) => {
            ...
            if (dragState.type === "move") {
                const nextX = Math.round(
                    Math.max(0, Math.min(100 - dragState.initialWidth, dragState.initialX + deltaPercentX)),
                );
                const nextY = Math.round(
                    Math.max(0, Math.min(100 - dragState.initialHeight, dragState.initialY + deltaPercentY)),
                );
                onUpdateComponent(dragState.compId, { x: nextX, y: nextY }, shouldRecord);
                ...
                if (dragState.isText && isCorner) {
                    ...
                    nextFontSize = Math.round(Math.max(10, Math.min(160, dragState.initialFontSize * s)));
                } else {
    ...
    const components = passedComponents ?? (slide ? getSlideComponents(slide) : []);

    // Helper render hình khối
    const renderShapeElement = (comp: SlideComponent) => {
        const shapeType = comp.shapeType || "rectangle";
        const fill = comp.fillColor === "transparent" ? "transparent" : comp.fillColor || "#c45b3f";
                ...
                {components.map(comp => {
                    ...
                    return (
                                ...
                                height:
                                    isShape && comp.shapeType !== "line"
                                        ...
                                        : comp.type === "image" && comp.height
                                          ? `${comp.height}%`
                                          : "auto",
                                minHeight: isText && comp.height ? `${comp.height}%` : undefined,
                                maxWidth: "100%",
                            }}
                        ...
                        >
                            ...
                            {comp.type === "shape" ? (
                                <div className="h-full w-full pointer-events-none">{renderShapeElement(comp)}</div>
                            ) : comp.type === "image" ? (
                                ...
                                <textarea
                                    ...
                                    style={{
                                        ...
                                        fontFamily: FONT_MAP[comp.fontFamily || "sans"] || comp.fontFamily || "inherit",
                                        textTransform: comp.textCase === "uppercase" ? "uppercase" : "none",
                                        lineHeight: 1.3,
                                    }}
                                ...
                                >
                                    {comp.type === "bullets" ? (
                                        ...
                                        <div className="whitespace-pre-wrap break-words">{comp.content}</div>
                                    )}
                                </div>
                            )}
                        </div>
